...
const SYSTEM_PROMPT = `你是一名面向外贸电商（特别是 TikTok）的市场研究员。你只输出 JSON，不输出 Markdown 或解释性文字。

给定一个产品（类目、目标国家/语言、产品事实）以及若干条来自 TikTok 的真实信号（高赞视频 caption / 评论），
请产出一份结构化市场调研，**必须**基于信号抽取，不能凭空想象：
...
function buildSearchKeywords(
  ...
  // 毛毯类目的经验关键词（其它类目可由运营在产品输入中给 search_keywords 覆写）
  if (productCategory.toLowerCase().includes("blanket")) {
    ["cozy blanket", "soft blanket", "tiktok blanket", "winter blanket"].forEach(
      (k) => out.add(k),
    );
  }
  if (typeof inp.material === "string") out.add(inp.material);
...
function mockResearch(category: string): ResearchStructured {
  return {
    ...
    competitors: [
      {
        name: "Sample Brand A",
        why_hot: "Uses ASMR texture close-ups",
        hook_used: "POV: you just got this blanket",
      },
    ],
    hot_hooks: [
      "POV: you unbox this",
