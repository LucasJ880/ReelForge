...
test("J4：UI 边界、API 非法输入、双击幂等、真实上传、50 图与取消", async ({
  ...
  const invalidCases = [
    {
      name: "51 images",
      body: { ...validBody, assetIds: acceptanceAssetIds(51) },
      status: 400,
    },
    ...
    { name: "negative", body: { ...validBody, requestedCount: -1 }, status: 400 },
    {
      ...
      body: {
        ...validBody,
        assetIds: [acceptanceAssetIds(1)[0], acceptanceAssetIds(1)[0]],
      },
      status: 409,
    },
  ];
...
test("J5：两个批次并行推进且各自通过 HTTP UI 验证", async ({
  ...
  const created = await page.evaluate(
    async ({ templateId, templateVersion, requestKeys, assetIdsByBatch }) =>
      Promise.all(
        requestKeys.map(async (key, batchIndex) => {
          const response = await fetch("/api/batches", {
            ...
            body: JSON.stringify({
              templateId,
              templateVersion,
              assetIds: assetIdsByBatch[batchIndex],
              requestedCount: 50,
            }),
          });
      ...
      ),
    {
      ...
      templateVersion: template.version,
      requestKeys: keys,
      assetIdsByBatch: keys.map(() => acceptanceAssetIds(20)),
    },
  );
  ...
  const dispatchPayload = await dispatchCurrentSingleVideo(
    singlePage,
    "15 秒中文 UGC 产品演示，突出耐用与便携，真实自然光，清晰展示使用前后。",
  );
