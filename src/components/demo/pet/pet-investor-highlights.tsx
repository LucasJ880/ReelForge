...
export function PetInvestorHighlights() {
  ...
  return (
    <section id="investor" className="relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0 -z-10 pet-ambient-glow opacity-60" />
      <div className="mx-auto w-full max-w-7xl px-5 py-16 sm:px-8 lg:px-10">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[color:var(--pet-orange)]">
          {s.eyebrow}
        </p>
        ...
        <dl className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {s.metrics.map((m) => (
            <div key={m.label} className="pet-surface rounded-3xl p-5">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {s.pillars.map((p) => (
            <div key={p.title} className="pet-surface rounded-3xl p-6">
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          ...
          <a
            ...
            className="inline-flex items-center rounded-full border border-border bg-card px-5 py-3 text-sm font-semibold text-foreground transition hover:bg-secondary"
          >
