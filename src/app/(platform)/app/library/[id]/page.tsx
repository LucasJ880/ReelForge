...
import { SubtitleDownloadLink } from "@/components/library/subtitle-download-link";
import { MakingProcessTimeline } from "@/components/library/making-process-timeline";
...
export default async function PlatformLibraryItemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const copy = getPlatformCopy(locale).library;
  return (
    <div className="editorial-page-stack">
      ...
      <header className="space-y-4"><div className="flex flex-wrap items-center gap-3"><h1 className="editorial-display">{item.title}</h1><Badge variant={item.status === "ready" ? "success" : item.status === "failed" ? "destructive" : "secondary"}>{copy.statuses[item.status]}</Badge>{item.isShowcase ? <Badge variant="secondary" title={copy.showcaseHint}>{copy.showcaseBadge}</Badge> : null}</div><p className="font-mono text-body tabular-nums text-muted-foreground">{item.durationSec ? `${item.durationSec}s` : copy.durationPending} · {item.aspectRatio ?? copy.aspectPending} · {item.id}</p>{item.isShowcase ? <p className="text-body text-muted-foreground">{copy.showcaseHint}</p> : null}{item.videoUrl || item.subtitleFileUrl || (item.briefId && !item.isShowcase) ? <div className="flex flex-wrap gap-2">{item.videoUrl ? <VideoDownloadLink videoUrl={item.videoUrl} filename={`aivora-${item.id}.mp4`} label={copy.download} /> : null}{item.subtitleFileUrl && !item.isShowcase ? <SubtitleDownloadLink subtitleFileUrl={item.subtitleFileUrl} filename={`aivora-${item.id}.srt`} label={copy.downloadSubtitles} /> : null}{item.briefId && !item.isShowcase ? <><VideoActions briefId={item.briefId} failedSceneCount={item.failedSceneCount} canRetry={item.canRetry} /><ReportContentButton briefId={item.briefId} /></> : null}</div> : null}{item.status === "failed" ? <p className="text-body text-muted-foreground">{copy.detailFailure}</p> : null}</header>
      <MakingProcessTimeline steps={item.makingProcess} english={locale === "en-US"} />
