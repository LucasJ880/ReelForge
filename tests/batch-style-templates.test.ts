...
test("Phase4：原创质量锁定模板扩容到至少 30 个，slug 唯一且覆盖至少 12 个场景分类", () => {
  assert.ok(BATCH_STYLE_TEMPLATE_SEEDS.length >= 30);
  assert.equal(
    new Set(BATCH_STYLE_TEMPLATE_SEEDS.map((template) => template.slug)).size,
    BATCH_STYLE_TEMPLATE_SEEDS.length,
  );
  assert.ok(new Set(BATCH_STYLE_TEMPLATE_SEEDS.map((template) => template.category)).size >= 12);
...
test("Phase5a：31+ 模板 × 20 组产品输入形成确定性的质量锁定矩阵", () => {
...
test("Phase2：重复 seed 不新增、不归档、不覆盖已存在 v2", async (t) => {
  let updateCalls = 0;
  let createCalls = 0;
  patchStyleTemplate(t, {
