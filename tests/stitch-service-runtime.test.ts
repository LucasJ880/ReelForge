...
test("claimStitchTask：CAS PENDING → STITCHING；并发时只有一条 runner 抢到", async (t) => {
  ...
  assert.ok(task1?.attemptToken, "claim 必须返回 attempt token");
...
test("claimStitchTask：单段 fv 也能被外部 runner 领取（孤儿任务回归）", async (t) => {
  ...
  assert.ok(task?.attemptToken, "单段 claim 也必须返回 attempt token");
  assert.equal(task?.segmentUrls.length, 1);
  assert.equal(updateManyCalls, 1, "应触发一次 CAS PENDING→STITCHING");
});

test("P0 回归：20+ 个较旧缺段 PENDING 不得饿死后续 ready claim", async (t) => {
  const blockers = Array.from({ length: 25 }, (_, index) => ({
    id: `fv_claim_blocker_${String(index).padStart(2, "0")}`,
    status: FinalVideoStatus.PENDING,
    segmentCount: 2,
    stitchAttempts: 0,
    targetDurationSec: 30,
    ...
    segments: [
      {
        segmentIndex: 0,
        status: VideoJobStatus.SUCCEEDED,
        outputVideoUrl: `https://cdn.example.com/blocker-${index}.mp4`,
        outputThumbUrl: null,
      },
    ],
  }));
  const ready = {
    id: "fv_claim_ready_after_blockers",
    status: FinalVideoStatus.PENDING,
    segmentCount: 2,
    stitchAttempts: 0,
    targetDurationSec: 30,
    ...
    segments: [
      {
        segmentIndex: 0,
        status: VideoJobStatus.SUCCEEDED,
        outputVideoUrl: "https://cdn.example.com/ready-0.mp4",
        outputThumbUrl: null,
      },
      {
        segmentIndex: 1,
        status: VideoJobStatus.SUCCEEDED,
        outputVideoUrl: "https://cdn.example.com/ready-1.mp4",
        outputThumbUrl: null,
      },
    ],
  };
  ...
  patchFinalVideo(t, {
    ...
    updateMany: (async (args: { where: { id: string } }) => {
      claimedId = args.where.id;
      return { count: 1 };
    }) as never,
  });

  const task = await claimStitchTask();
  assert.equal(task?.finalVideoId, ready.id);
  assert.equal(claimedId, ready.id);
  assert.equal(findManyCalls.length, 2, "claim 必须翻到第二页而非返回 null");
  assert.deepEqual(
    (findManyCalls[1]?.cursor as { id: string } | undefined)?.id,
    blockers[19].id,
  );
  assert.equal(findManyCalls[1]?.skip, 1);
});
...
test("finishStitchTask：写 stitchedVideoUrl + status=READY", async (t) => {
  const fakeFv = {
    ...
    targetDurationSec: 30,
    thumbnailUrl: null,
    stitchAttemptToken: "attempt-done",
    brief: null,
  };
  ...
  assert.equal(updateManyCalls[0]?.data.status, FinalVideoStatus.READY);
  assert.equal(
    updateManyCalls[0]?.data.stitchedVideoUrl,
    "https://cdn.example.com/final.mp4",
  );
  assert.equal(updateManyCalls[0]?.data.thumbnailUrl, "https://cdn.example.com/final.jpg");
  assert.equal(updateManyCalls[0]?.data.ffmpegError, null);
  assert.equal(updateManyCalls[0]?.data.stitchAttemptToken, null);
});
...
test("finishStitchTask：error 入参 → 写 ffmpegError + status=FAILED + attempts++", async (t) => {
  const fakeFv = {
    ...
    targetDurationSec: 30,
    thumbnailUrl: null,
    stitchAttemptToken: "attempt-fail",
    brief: null,
  };
  const updateManyCalls: Array<{ where: Record<string, unknown>; data: Record<string, unknown> }> = [];
  ...
  assert.equal(updateManyCalls[0]?.data.status, FinalVideoStatus.FAILED);
  assert.equal(updateManyCalls[0]?.data.ffmpegError, "ffmpeg crashed");
  assert.deepEqual(updateManyCalls[0]?.data.stitchAttempts, { increment: 1 });
  assert.equal(updateManyCalls[0]?.data.stitchAttemptToken, null);
});
