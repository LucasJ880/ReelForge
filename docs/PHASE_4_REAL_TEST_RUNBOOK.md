...
| 场景 | 段数 | OpenAI | Seedance | Blob | Total |
|---|---|---|---|---|---|
| **A · Personal 15s 文生视频** | 1 | ~$0.10 (gpt-5.5 director+script+videoPrompt) | ~$0.50 (1 段 1080p 15s) | $0 (1 段不走 stitch) | **≈ $0.6** |
| **B · Personal 30s 文生视频** | 2 | ~$0.15 | ~$1.00 | ~$0.001 (1 GH Action stitch upload) | **≈ $1.2** |
| **C · Business 30s + auto end card** | 2 AI + 1 end card | ~$0.20 | ~$1.00 | 可能 ~$0.05 (logo gen 走 OpenAI Image，缺 logo 时) | **≈ $1.3** |

> Seedance 真实计费按秒，1080p 大约 0.32 RMB/s ≈ $0.044/s。15s ≈ $0.66。
> 数字仅作量级参考；以你火山方舟控制台的实际账单为准。
...
🔥 Seedance      = REAL (doubao-seedance-2-0-260128)
🔥 OpenAI Image  = REAL (gpt-image-1)
…
```
...
人为让一段失败（如把 ARK_API_KEY 临时换成无效值）：
