  ...
  return (
    ...
    >
      <div className="pet-surface rounded-3xl p-5 sm:p-6">
        <div className="grid items-center gap-5 lg:grid-cols-[1fr_auto_1fr]">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <PhoneVideoMockup
              ...
              fallbackGradient="from-stone-400/40 via-stone-300/30 to-stone-200/20"
              fallbackTitle="原始素材"
          ...
          <div className="flex flex-col items-center gap-3 py-2">
            <span className="hidden text-(--pet-orange) lg:block">
              <ArrowRight size={28} />
            </span>
            <span className="text-(--pet-orange) lg:hidden">
            ...
            <div className="flex flex-col items-center gap-1.5">
              {b.aiSteps.map((s) => (
                <span
                  key={s}
                  className="inline-flex items-center gap-1.5 rounded-full border border-(--pet-orange)/25 bg-(--pet-orange)/8 px-3 py-1 text-[11px] font-semibold text-(--pet-orange)"
                >
                  <Sparkles size={11} /> {s}
                </span>
              ))}
          ...
          {/* After */}
          <div className="flex flex-col items-center gap-3">
            <span className="rounded-full border border-(--pet-orange)/30 bg-(--pet-orange)/10 px-3 py-1 text-xs font-semibold text-(--pet-orange)">
              {b.after.label}
            </span>
            <PhoneVideoMockup
              sizeClassName="h-[420px] w-full max-w-[236px]"
            ...
            <ul className="w-full max-w-[260px] space-y-1.5">
              {b.after.notes.map((n) => (
                <li
                  key={n}
                  className="flex items-start gap-2 text-[11px] leading-5 text-foreground/80"
                >
                  <Sparkles
                    size={12}
                    className="mt-0.5 shrink-0 text-(--pet-teal)"
                  />
                  {n}
                </li>
              ))}
        ...
        <p className="mt-5 text-center text-[11px] leading-5 text-muted-foreground/80">
          {b.note}
        </p>
      </div>
    </PetSection>
