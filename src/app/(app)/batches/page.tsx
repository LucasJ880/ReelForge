...
const statusConfig: Record<string, { label: string; dot: string }> = {
  PENDING: { label: "等待中", dot: "bg-muted-foreground/60" },
  RUNNING: { label: "执行中", dot: "bg-primary animate-pulse" },
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/batches/new"
          className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-white transition-colors hover:opacity-90"
        >
          ...
          {batches.map((batch) => {
            ...
            return (
              <Link key={batch.id} href={`/batches/${batch.id}`}>
                <div className="group flex items-center justify-between rounded-xl border border-white/5 bg-card/60 p-4 transition-all hover:border-border hover:bg-white/5">
