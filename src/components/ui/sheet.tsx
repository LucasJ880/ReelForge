...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-overlay transition-opacity duration-fast ease-out data-ending-style:opacity-0 data-starting-style:opacity-0 motion-reduce:transition-none",
        className
      )}
      {...props}
    />
    ...
    <SheetPortal>
      ...
      >
        {side === "bottom" ? (
          <div
            aria-hidden
            className="mx-auto mt-3 mb-1 h-1.5 w-10 shrink-0 rounded-full bg-border"
          />
        ) : null}
        {children}
        {showCloseButton && (
...
function SheetTitle({ className, ...props }: SheetPrimitive.Title.Props) {
  return (
    <SheetPrimitive.Title
      ...
      className={cn(
        "font-heading text-title font-normal text-foreground",
        className
      )}
      {...props}
    />
