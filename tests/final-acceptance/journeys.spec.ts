...
test("J5：两个批次并行推进且各自通过 HTTP UI 验证", async ({
  ...
  await singlePage.goto("/personal/create-video");
  await singlePage
    .getByLabel("创意需求")
  ...
  await singlePage
    .getByRole("button", { name: "第一步：生成脚本" })
    .click();
  ...
  await expect(singlePage.getByText(/1 支成片任务已开始生成/)).toBeVisible();
  let singleSummary:
  ...
  await expect(singlePage).toHaveURL(/\/personal\/videos\?highlight=/);
